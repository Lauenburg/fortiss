...
function drawMonthChart(timeMonth, chartTypMonth) {
    ...
    if (chartTypMonth === 0 || chartTypMonth === undefined) {
    ...
    */
    ...
    {
        if(!aggregiertMonth){
            hideButtonDisaggregateConsumersMonth()
        }else{
            showButtonDisaggregateConsumersMonth()
        }
...
function drawMonthChartBlock(showThreePhase) {
    ...
    var y = document.getElementById("aggregateButtonMonth");
...
*/
    d3.json("https://" + connection + ":"+port+"/get/" + filID + "/getMonth/" + monthString  + "/").then(function (dataBrick) {

        var monthBlockcolor = color;

        if (noData(dataBrick,"#MonthChart",2)) {
            firstRunMonth = true;
            svgMonth.selectAll("g").remove();
            svgMonth.selectAll("p").remove();
        ...
        //Important: nameBricksMonth is global
    ...
    */
        ...
        if (showThreePhase) {
            monthBlockcolor = colorAggregated;
            console.log("I do get here"+showThreePhase)
            for (var i = 0; i < dataJson.length; i++) {
                dataJson[i] = {
                    x: dataJson[i].x, y: parseInt(dataJson[i]["sum"]), sum: dataJson[i]["sum"], key: "PH1 + PH2 + PH3"
                }
            }
            nameBricksMonth = []
        ...
        var z = d3.scaleOrdinal().domain(nameBricksMonth).range(monthBlockcolor)
        ...
        //fügt der x achse den namen zu
            ...
            .on("mousemove", function (d) {
                ...
                tooltip.select("text").text(d[1]-d[0]).style("font-size","15px");
            });
    }
...
function drawMonthChartPi(aggregiert){
    if(!aggregiert) {
        ...
        */
        ...
        getDataMonthAndDrawPiChart(g1, (widthMonthSvg * 0.2 +100 - radius), height / 5, "legendOrdinal1", "PH1", colorPerPhase[0]);
...
function getDataMonthAndDrawPiChart(g,positionX,positionY,legendID,phaseID, colorScheme) {
    ...
    d3.json("https://" + connection + ":"+port+"/get/" + filID + "/getMonth/" + monthString  + "/").then(function (dataBrick) {
        /*
         This function checks if the array is empty. Setting SUM,MAX,AVG and Cost to "/" and exiting drawMonthChartBlock()
         */
        if(noData(dataBrick,"#MonthChart",2)) {
            firstRunMonth = true;
            svgMonth.selectAll("g").remove();
        ...
        */
        ...
        SettingDashboardvalues(dataJson, 2, pricePerWh)
            ...
            if (disaggreConsumersMonth) {
                ...
                ordinalMonth = d3.scaleOrdinal().domain(nameBricksMonth).range(colorGreyBlue);
                var label = d3.arc()
                ...
                if (!sumOfPhaseData[0].name.localeCompare("No_Data")) {
                    ...
                    ordinalMonth = d3.scaleOrdinal().domain(["Sum = 0"]).range(colorGrey);
                    var label = d3.arc()
                        .outerRadius(0)
                        .innerRadius(0);
