...
function requestDayData(num) {
    ...
    var path = "/get/"+filID+"/getDay/" + dayString  + "/";
...
function updateDayValue(result){
    ...
    if(result.length >0) {
        var historyDataDays = $('.historyDataDay');
        var wrapperDays = $('.wrapperDay')
        wrapperDays.each(function (index, element) {
        ...
        historyDataDays.each(function (i, e) {
            if(i < result.length) {
